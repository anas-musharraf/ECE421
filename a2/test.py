...
class TestCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        #`print('\nTesting loss: {}, acc: {}\n'.format(loss, acc))
        if 'test_loss' not in self.model.history.history.keys():
            self.model.history.history['test_loss'] = []
            self.model.history.history['test_acc'] = []
        self.model.history.history['test_loss'].append(loss)
        self.model.history.history['test_acc'].append(acc)
...
def conv_neural_network():
   ...
   # model.add(tf.keras.layers.Dense(784, activation=tf.nn.relu, kernel_regularizer=tf.keras.regularizers.l2(0.5)))
    model.add(tf.keras.layers.Dense(10, activation=tf.nn.softmax))
    model.compile(optimizer=tf.keras.optimizers.Adam(lr=0.0001), loss='categorical_crossentropy',metrics=['accuracy'])
    model.fit(trainData, newTrain, batch_size=32, epochs=50, validation_data=(validData, newValid), callbacks=[TestCallback((testData, newTest))], verbose=1)
    ...
    test_loss = model.history.history['test_loss']
    test_acc = model.history.history['test_acc']
    print("()()()()()()()")
    print(train_acc[-1], val_acc[-1], test_acc[1])
    print("KDJNFLJKNDSLKJF")
    ...
    plt.ylabel('Loss')
    plt.xlabel('Epoch')
    plt.title('Loss vs Epoch Using SGD, Dropout p=0.9')
    plt.legend(loc='best')
    plt.savefig('2_3_2_loss_vs_epoch_SGD.png')
    plt.show()
'''
    ...
    plt.ylabel('Accuracy')
    plt.xlabel('Epoch')
    plt.title('Accuracy vs Epoch Using SGD, Dropout p=0.75')
    plt.legend(loc='best')
    plt.savefig('2_3_2_accuracy_vs_epoch_dropout75.png')
    plt.show()
    return 0
